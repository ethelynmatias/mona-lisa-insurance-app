...
export default function WebhookHistoryPanel({ webhooks = [], showFormColumn = false, clearRoute = null }) {
    ...
    function handleDeleteEntry(webhook) {
        const entryInfo = webhook.entry_id 
            ? `Entry ID: ${webhook.entry_id}` 
            : `Entry from ${webhook.form_name || webhook.form_id}`;
        
        if (!confirm(`Delete this webhook entry (${entryInfo})? This cannot be undone.`)) return;
        
        router.delete(route('webhook.history.delete', { log: webhook.id }), { 
            preserveScroll: true 
        });
    }
